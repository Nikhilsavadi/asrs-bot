"""
backtest_spy.py — ASRS Strategy on SPY via Polygon.io
═══════════════════════════════════════════════════════════════════════════════

Adapts the DAX bar-5 breakout strategy to SPY:
  - RTH: 09:30–16:00 ET (vs 09:00–17:30 CET for DAX)
  - Bar 5 = 09:55–09:55 ET (4th 5-min candle after open)
  - Pre-market range (04:00–09:30 ET) replaces overnight range (00:00–06:00 CET)
  - Thresholds scaled for SPY price level (~$500 vs ~17000 DAX)

Usage:
    python backtest_spy.py                   Full backtest
    python backtest_spy.py --no-overnight    Without pre-market bias filter
"""
...
logger = logging.getLogger("SPY_BT")

POLYGON_KEY = os.getenv("POLYGON_API_KEY", "arL6Kqp4GoBiLF_x97ovrFeHYS7ilN80")
DATA_DIR = os.path.join(os.path.dirname(__file__), "data", "spy")

# ── SPY-scaled thresholds ────────────────────────────────────────────────────
...
NARROW_RANGE  = 0.50     # Bar 5 range < $0.50 = narrow
WIDE_RANGE    = 2.00     # Bar 5 range > $2.00 = wide
...
async def fetch_spy_5min(start_date: str, end_date: str, extended: bool = False) -> pd.DataFrame:
    """
    Fetch SPY 5-min bars from Polygon. Handles pagination.
    If extended=True, includes pre/post-market hours.
    """
    cache_name = f"spy_5min_{'ext' if extended else 'rth'}_{start_date}_{end_date}.parquet"
    ...
    all_results = []
    url = f"https://api.polygon.io/v2/aggs/ticker/SPY/range/5/minute/{start_date}/{end_date}"
...
# ══════════════════════════════════════════════════════════════════════════════
#  BACKTEST ENGINE — Same logic as DAX, adapted for SPY
# ══════════════════════════════════════════════════════════════════════════════
...
def calculate_premarket_bias(
    ext_df: pd.DataFrame, trade_date, bar5_high: float, bar5_low: float
) -> str:
    """
    ...
    try:
        ...
        if bar5_low >= pm_high:
            return "SHORT_ONLY"
        elif bar5_high <= pm_low:
            return "LONG_ONLY"
        else:
            # Partial overlap — check how much is outside
            if bar5_high > pm_high and bar5_low > pm_low:
                above_pct = (bar5_high - pm_high) / bar5_range
                if above_pct > 0.75:
                    return "SHORT_ONLY"
            elif bar5_low < pm_low and bar5_high < pm_high:
                below_pct = (pm_low - bar5_low) / bar5_range
                if below_pct > 0.75:
                    return "LONG_ONLY"
            return "STANDARD"
    except Exception:
...
def run_backtest(
    ...
    """Run ASRS backtest on SPY 5-min data. Same logic as DAX."""
    results = []
    prev_close = 0
            ...
            if direction == "LONG":
                ...
                if row["Low"] <= trail_stop:
                    ...
                    day_result.trades.append(Trade(
                        ...
                        held_bars=i - entry_bar_idx, bar_range=bar5_range,
...
def print_results(results: list[DayResult], label: str = ""):
    ...
    print(f"\n{'═' * 70}")
    print(f"  ASRS BACKTEST ON SPY — {label}" if label else f"  ASRS BACKTEST ON SPY")
    print(f"{'═' * 70}")
...
async def main():
    ...
    logger.info(f"Fetching SPY 5-min data: {start_date} to {end_date}")

    # Fetch RTH and extended hours
    rth_df = await fetch_spy_5min(start_date, end_date, extended=False)
    if rth_df.empty:
        return

    ext_df = None
    if use_overnight:
        ext_df = await fetch_spy_5min(start_date, end_date, extended=True)

    logger.info(f"Running ASRS backtest on SPY ({len(rth_df)} RTH bars)...")
